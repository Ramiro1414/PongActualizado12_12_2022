...
begin
    ...
    while (not GameFinish) do
        begin
            if (GolPlayerDer = 5) or (GolPlayerIzq = 5) then
                    begin
                        GameFinish:= True;
                    end
                    else
    ...
    if GolPlayerDer = 5 then
        begin
            OutTextXY(0,0,'Gana el jugador de la derecha.');
        end;
    if GolPlayerIzq = 5 then
        begin
            OutTextXY(0,0,'Gana el jugador de la izquierda.');
        end;
    OutTextXY(0,30,'Presione <BACKSPACE> para regresar.');
while (leetecla <> #8) do
        begin
            leetecla:= readkey;
        end;
GolPlayerDer:= 0; // Los goles deben volver a 0 despues de cada partida.
GolPlayerIzq:= 0;
end;

procedure PONG1VSIA_FACIL();
var
GameFinish: boolean = False; // GameFinish es una booleana que indica el estado de la partida. Si es False, la partida está en curso. Cuando es True, la partida terminó.
begin
    dy:=valordy;
    dx:= valordx;
    Dificultad:= 430;
    DIBUJACANCHA();
    While not GameFinish do // Mientras GameFinish = False, se juega la partida, porque está en curso. Si es True, la partida termina.
        begin
            if (GolPlayerDer = 5) or (GolPlayerIzq = 5) then // Si algun jugador llega a 5 goles, GameFinish = True, entonces, termina la partida.
                begin
                    GameFinish:= True;
                end
                else
    ...
    ClearDevice;
// Esta parte de programa se utiliza para mostrar en pantalla al ganador de la partida.
    if GolPlayerDer = 5 then
        begin
            OutTextXY(0,0,'Gana el jugador de la derecha.');
        end;
...
// Finalmente, se pide que el usuario presione BACKSPACE para volver al Menu de seleccion de modo de juego.
OutTextXY(0,30,'Presione <BACKSPACE> para regresar.');
while (leetecla <> #8) do
        begin
            leetecla:= readkey;
